...
class GoodsFeatureEditor extends Grid {
	...
	protected function loadData() {
		if ($this->getState() == 'getRawData') {
			...
			$features = (!empty($_SESSION['goods_feature_editor']['filter_feature_id'])) ? $_SESSION['goods_feature_editor']['filter_feature_id'] : array('-1');
	...
	protected function main() {
		...
		if ($smapID) {
			$features = $this->dbh->getColumn('shop_sitemap2features', 'feature_id', array('smap_id' => $smapID));
			if ($features) {
				foreach ($features as $feature_id) {
					if ($goodsID) {
						$fpv_id = $this->dbh->getScalar('shop_feature2good_values', 'fpv_id', array('feature_id' => $feature_id, 'goods_id' => $goodsID));
					} else {
						$fpv_id = $this->dbh->getScalar('shop_feature2good_values', 'fpv_id', array('feature_id' => $feature_id, 'session_id' => session_id()));
					}
					if (!$fpv_id) {
						$fpv_id = $this->dbh->modify(
							QAL::INSERT_IGNORE, 'shop_feature2good_values',
							array('feature_id' => $feature_id, 'goods_id' => $goodsID, 'session_id' => session_id())
						);
						if ($fpv_id) {
							foreach ($languages as $lang_id => $language) {
								$this->dbh->modify(
									...
									array('fpv_id' => $fpv_id, 'lang_id' => $lang_id));
							}
						}
					}
				}
